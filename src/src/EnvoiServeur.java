 ...
 */
public class EnvoiServeur extends Thread{
	 ...
	 * @param out Flux dans lequel on envoi les données
	 * @param jeu classe dans laquelle est créé l'objet et dans laquelle est située la requete à envoyer
	 */
	public EnvoiServeur(PrintStream out,JeuServeur jeu){
		this.out=out;
		this.jeu=jeu;
	}
	...
	public void run(){
		while(true){
			if(jeu.getRequete()!=null){
				...
				if(jeu.getRequete().equals("p")) {jeu.getFenetre().pannel.envoip=false;}
			jeu.setRequete(null);
			}
		}
	}
